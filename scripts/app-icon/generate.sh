#!/usr/bin/env bash
#
# icon.svg から AppIcon.appiconset の 1024x1024 PNG（通常・ダーク・ティント）を生成する。
# アイコンの意匠を変更したら icon.svg を編集して本スクリプトを再実行する。
#
set -euo pipefail
...
readonly APPICONSET="$PROJECT_ROOT/AuthWithWebView/Assets.xcassets/AppIcon.appiconset"
readonly SIZE=1024

# icon.svg の配色。ダーク / ティントはこの 2 色を置き換えて生成する
readonly LIGHT_BACKGROUND="#00695F"
readonly LIGHT_FOREGROUND="#FFFFFF"

command -v rsvg-convert >/dev/null || {
  echo "❌ rsvg-convert が見つかりません" >&2
  echo "   brew install librsvg" >&2
  exit 1
}

[[ -f "$SOURCE" ]] || { echo "❌ $SOURCE が見つかりません" >&2; exit 1; }
[[ -d "$APPICONSET" ]] || { echo "❌ $APPICONSET が見つかりません" >&2; exit 1; }
...
# ダークは web/app/theme.ts のダークテーマ primary に合わせる
